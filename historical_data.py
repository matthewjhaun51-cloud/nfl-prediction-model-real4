...
def backfill(seasons, weeks, key, progress=None):
    out=Path("data/historical"); out.mkdir(parents=True,exist_ok=True); results=[]
    for season in seasons:
        status={"season":season,"success":0,"skipped":0,"errors":[]}; blocked=False
        for week in weeks:
            ...
            if blocked:
                ...
                continue
            try:
                players=pd.DataFrame(player_stats_week(season,week,key)); teams=pd.DataFrame(team_stats_week(season,week,key))
                players.to_parquet(pf,index=False); teams.to_parquet(tf,index=False); status["success"]+=1
                if progress: progress(season,week,"success",None)
            except SportsDataError as e:
                if e.status_code in (401,403):
                    blocked=True; status["skipped"]+=1; status["errors"].append(f"Week {week}: HTTP {e.status_code} unauthorized/forbidden")
