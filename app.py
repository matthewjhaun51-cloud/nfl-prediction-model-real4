...
if st.button('⬇️ Build/Update REAL historical data',use_container_width=True):
    ...
    def prog(s,w,status=None,detail=None): state['n']+=1; bar.progress(min(state['n']/total,1.0))
    with st.spinner('Downloading real team and player game data...'): results=backfill(seasons,range(1,19),sports_key,prog)
    st.success('Historical data cache updated. No synthetic player/team statistics were generated.')
    for r in results:
        if r['errors']:
            st.warning(f"{r['season']}: {r['success']} weeks available/cached; {r['skipped']} weeks skipped because SportsDataIO returned HTTP 401/403.")
            st.caption(' | '.join(r['errors']))
        else: st.info(f"{r['season']}: {r['success']} weeks available/cached.")
def norm_team(name):
