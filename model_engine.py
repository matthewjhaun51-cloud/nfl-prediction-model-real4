...
def build_team_ratings(d,season,week):
    if d.empty:return {}
    s=pd.to_numeric(d.get('Season'),errors='coerce'); w=pd.to_numeric(d.get('Week'),errors='coerce'); d=d[(s<season)|((s==season)&(w<week))].copy()
    if d.empty:return {}
    ...
    def f(g):
        wt=np.sqrt(g.w); return {'margin':np.average(g.PointDifferential,weights=wt),'ypp':np.average(g.OffensiveYardsPerPlay,weights=wt),'turnover':np.average(g.Turnovers-g.OpponentTurnovers,weights=wt),'pass':np.average(g.PassingYards,weights=wt),'rush':np.average(g.RushingYards,weights=wt)}
